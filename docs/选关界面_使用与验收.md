...
构建：`fd06977` + R014 工作区（未提交）；状态：自动验证通过，待手动验收。手机当前已安装的 R013 包不包含本轮音效修改。

点击音效集中配置于 `res://game/autoload/sound_manager.tscn` 的 `UIClick`，音源为 `res://assets/audio/ui/mouseclick1.ogg`，SFX 总线、默认 0 dB、音调 1、最大并发 4。可在 Inspector 修改音量。`play_ui_click()` 负责播放；普通菜单回调、触摸和文字提示在有效操作入口调用，焦点更新不调用。SoundManager 始终处理，战场慢动作/静音不控制此播放器。

| 用例 | 操作 | 预期 | 实际 |
| --- | --- | --- | --- |
| U01 | 标题点击进入选关，点击编号、已选编号与开始 | 每次有效确认一次点击音效，切场景不截断 | 未验收 |
| U02 | 方向导航、点击 4–9 与空白、按下后移出或取消触摸 | 无点击音效 | 未验收 |
| U03 | 点击试飞顶部开始提示，尝试重复点击 | 只接受一次开始并播放一次；其他飞行/攻击操作不添加 UI 音效 | 未验收 |
| U04 | 暂停后等慢动作结束，继续/重试/返回选关；使用 Esc 和手柄 Start | 菜单操作声音正常、一次一声；Start 恢复试飞不意外开始挑战 | 未验收 |
| U05 | 切后台再回前台 | 自动暂停无点击声，手动继续有点击声 | 未验收 |
| U06 | 成功/失败动画前点击，再等提示出现点击返回 | 动画前无点击声，允许返回后播放一次；结算静音不影响 UI 声音 | 未验收 |
| U07 | 快速连点关卡，分别用触屏、鼠标、键盘及物理手柄确认 | 单次操作不双播，音量合适，与胜负音效和 BGM 配合正常 | 未验收 |

记录构建、设备与系统、用例编号、通过/失败及复现步骤；不能检查写未验收。自动测试 32 项点击音效检查及全部既有回归通过；日志见 P1 文档 R014。初次失败及修复记录保留，听感不能以 Headless 自动测试替代。

## R015：暂停菜单尺寸验收

构建：`8fe0cdf` + R015 工作区（未提交），待手动验收。暂停菜单 Continue、Retry、Select mission 已对齐选关 Start 的 360×128 最小尺寸和字号 40，竖排间距 16。右上角 Pause 保持原样，配色、焦点、模糊与点击音效不变；本轮未更新手机安装包。

电脑与 iPhone 横屏分别打开试飞/挑战的暂停菜单，检查三项文字完整、按钮等宽居中、间距清楚；逐个点击中部及四边内侧，确认可操作且一次一声；检查继续、重试和返回选关行为以及右上角 Pause 位置未变。iPhone 重点比较与选关开始按钮的大小及可点性，检查刘海/圆角安全区域。所有真人项目前均未验收，按构建、设备/系统、步骤、预期、实际结果记录。

自动回归与图形预览通过，详见 P1 文档 R015；现有点击音量 -18 dB 保留。

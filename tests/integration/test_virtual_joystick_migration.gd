...
const MAIN_GAME_SCENE := "res://game/core/main_game.tscn"
...
func _ready() -> void:
	...
	var packed_scene := load(MAIN_GAME_SCENE) as PackedScene
	_expect(packed_scene != null, "main game scene loads")
	if packed_scene != null:
		var main_game := packed_scene.instantiate()
		var player := main_game.get_node("Player") as Player

		var virtual_joystick := main_game.get_node_or_null("GameHUD/PlayerTouchControl/MoveJoystick")
